    ...
    for obj in bucket.objects.all():            
        ...
        if not key.endswith('/'):
            try:
                ...
                s3_client.put_object(Bucket='%smasked'%bucket_nm, Key='%s_masked'%file_name, Body=fake_handle.read())
                
                """move original files to backup filder"
                s3_resource = boto3.resource('s3')
                s3.Object('%sunmasked'%bucket_nm,'%s_unmasked'%file_name).copy_from(CopySource='%s/%s'%(bucket_nm,file_name))
